...
use crate::modules::button::ButtonText::{Abort, Detonate, Hold};
...
impl Module for ButtonModule {
    ...
    fn solve(&self, edgework: Edgework) -> Self::Output {
        if matches!(self.colour, Blue) && matches!(self.text, Abort) {
            "Hold"
            ...
            "Hold"
        } else if matches!(self.colour, Red) && matches!(self.text, Hold) {
            "Release"
        } else {
            "Hold"
        }.to_string()
